...
COLORS_SNOWFLAKE_LIST = tuple(t[0] for t in COLORS)
...
class ColorButton(disnake.ui.Button['ColorView']):
    ...
    async def callback(self, interaction: disnake.MessageInteraction):
        ...
        if int(self.custom_id) in interaction.author._roles:
            # await interaction.response.send_message(f'Убрана роль <@&{self.custom_id}>', ephemeral=True)
            await interaction.author.remove_roles(disnake.Object(self.custom_id))
            return

        # await interaction.response.send_message(f'Добавлена роль <@&{self.custom_id}>', ephemeral=True)
        await interaction.author.add_roles(disnake.Object(self.custom_id))
        await interaction.author.remove_roles(*filter(lambda x: x.id in COLORS_SNOWFLAKE_LIST and str(x) != self.custom_id, interaction.author.roles))
...
class ColorChanger(commands.Cog, name='Цвета'):
    def __init__(self, bot: Bot) -> None:
        self.bot = bot
        self.view = None
